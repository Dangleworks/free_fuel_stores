...
function onVehicleSpawn(vehicle_id, peer_id, x, y, z, cost)
    ...
    if is_valid then
        g_savedata.fuel[vehicle_id] = resource_id
        --table.insert(g_savedata.fuel, vehicle_id, resource_id)
    end
end

function onTick(game_ticks)
	for vehicle_id,resource_id in pairs(g_savedata.fuel) do
		server.setVehicleTank(vehicle_id, "Tank", 100, resource_id)
	end
end
...
function onCustomCommand(full_message, user_peer_id, is_admin, is_auth, command, ...)
	...
	if command == "?test" then
		server.announce("LOLOLOL", json.stringify(g_savedata))
	end
end
